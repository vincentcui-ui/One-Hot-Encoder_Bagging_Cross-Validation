...
class OneHotEncoder(Encoder):
    ...
    def fit(self, data: np.ndarray) -> Encoder:
        ...
        for i in range(data.shape[1]):
            # >>> YOUR CODE HERE >>>
            unique_categories = np.unique(data[:, i])
            self.categories.append(unique_categories)
            # <<< END OF YOUR CODE <<<
    ...
    def encode(self, data: np.ndarray) -> np.ndarray:
        ...
        for i in range(data.shape[1]): # for each column in the input data
            ...
            for j in range(data.shape[0]): # for each row in the current column
                ...
                # >>> YOUR CODE HERE >>>
                category_indices = np.where(self.categories[i] == data[j, i])
                # <<< END OF YOUR CODE <<<

                # set the cell in the one-hot encoded data at the corresponding row and category index to 1
                # >>> YOUR CODE HERE >>>
                if category_indices[0].size > 0:
                    category_index = category_indices[0][0]
