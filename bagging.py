...
class Bagging(BaseEnsemble):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        for learner in self.learners:
            # >>> YOUR CODE HERE >>>
            sample_indices = self._prepare_sample(X_encoded)
            learner.fit(X_encoded[sample_indices], y[sample_indices])
            # learner.fit(...)
            # <<< END OF YOUR CODE <<<
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        # >>> YOUR CODE HERE >>>
        predictions = np.array([learner.predict(X_encoded) for learner in self.learners]).T
        # <<< END OF YOUR CODE <<<

        # Count the number of votes for each label
        # >>> YOUR CODE HERE >>>
        counts = [np.bincount(row) for row in predictions]
        # <<< END OF YOUR CODE <<<

        # Return the label with the most votes
        # >>> YOUR CODE HERE >>>
        y_pred = np.array([np.argmax(row) for row in counts])
        # <<< END OF YOUR CODE <<<
        return y_pred
